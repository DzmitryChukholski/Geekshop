window.onload = function() {
    ...
    var total_forms = parseInt($('ordertotalquantity').text()) || 0;
    ...
    function deleteOrderItem(row) {
        var target_name = row[0].querySelector(selectors: 'input[type="number"]').name;
        orderitem_num = parseInt(target.name.replace('orderitems-','').replace('-quantity',''));
        delta_quantity = -quantity_arr[orderitem_num];
        orderSummaryUpdate(price_arr[orderitem_num], delta_quantity);
    }
   ...
   $('.order_form select').change(function () {
        ...
        var orderitem_product_pk = target.options[tarhet.selectedIndex].value;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.price){
                        ...
                        var price_html = '<span>' + data.price.toString().replace(searchValue: '.', replaceValue: ',') + '</span> руб';
